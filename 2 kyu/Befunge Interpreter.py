def interpret(code):
    ...
    bDict = {
        ...
        '?' : random.choice([lambda x, y: [x, y + 1], lambda x, y: [x, y - 1], lambda x, y: [x + 1, y], lambda x, y: [x - 1, y]], p =[0,0,.5,.5]),
        
        '_' : lambda stack: [stack[1:], '>'] if stack[0] == 0 else [stack[1:], '<'], 
        '|' : lambda stack: [stack[1:], 'v'] if stack[0] == 0 else [stack[1:], '^'],
        ...
        'p' : lambda stack: [stack[3:], stack[0], stack[1], stack[2]],
        'g' : lambda stack: [ord(arr[stack[0]][stack[1]])] + stack[2:],
    ...
    while( current != '@' ):
        ...
        if current in ['>', '<', 'v', '^']:
            ...
            [stack, a, b, v] = bDict[current](stack)
            arr[a][b] = chr(v)
        elif current == 'g':
            stack = bDict[current](stack)
        elif current == ' ':
